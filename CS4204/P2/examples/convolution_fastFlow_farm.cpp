...
// Override the max thread count for performance testing
...
string_p get_image_name(int i) {
    string_p image_n = new std::string("../examples/images/image" +
                                       std::to_string(i) + ".png");
...
int main(int argc, char *argv[]) {
    ...
    unsigned int nr_images, worker_count, pattern, do_chunking, min_chunk_size;
    int i = 0;

    if (argc < 3) {
        std::cerr << "use: " << argv[0] << " <nrImages> <worker_count>"
                  << std::endl; // <imageSize> [<chunking>]\n ";
        return 1;
    }

    dim = 1024; // atoi(argv[1]);
    nr_images = atoi(argv[1]);
    ...
    std::cerr << "STARTING WITH nrImages: " << nr_images
              << ", imageSize: " << dim << ", worker_count: " << worker_count
              << std::endl;
    double beginning = get_current_time();

    struct Worker : public ff::ff_node_t<task_t, ushort *> {
        ushort **svc(task_t *t) {
            ushort **res = (ushort **)calloc(1, sizeof(ushort *));
            *res = process_image(*t);
            return res;
        }
    };

    struct GenerateInput : public ff::ff_node {
      private:
        int n;
        int *N;

      public:
        GenerateInput(int n) : n(n) {
        ...
        void *svc(void *t) {
            for (int i = 0; i < n; i++) {
                auto image_name_p = get_image_name(N[i]);
                task_t *task = (task_t *)malloc(sizeof(task_t));
                *task = read_image_and_mask(image_name_p);
                ff_send_out(task);
            }
            return (EOS);
        }
    ...
    ff::ff_farm<> farm(false, ff::ff_ofarm::DEF_IN_BUFF_ENTRIES,
                       ff::ff_ofarm::DEF_OUT_BUFF_ENTRIES, false, 10000, false);
    ...
    farm.add_emitter(new GenerateInput(nr_images));

    farm.run_and_wait_end();

    double end = get_current_time();

    std::cerr << "Runtime is " << end - beginning << std::endl;
    std::cout << end - beginning << std::endl;
