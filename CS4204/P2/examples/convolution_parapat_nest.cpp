...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < nr_images; i++) {
        input_queue.push(N[i]);
    }

    Worker<int, string_p> *getNameWorker = new Worker(&get_image_name);

    Worker<string_p, task_t> *readAndMaskWorker =
        new Worker(&read_image_and_mask);

    Farm<task_t, ushort *> *processFarm =
        new Farm(worker_count, &process_image);

    Pipe<int, string_p, task_t, ushort *> pipe(getNameWorker, readAndMaskWorker,
                                               processFarm, input_queue);
