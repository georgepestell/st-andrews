...
prop_cKind_emptyContext t = evaluate (cKind 0 [] t Star)
...
spec :: Spec
spec = do
  describe "Test checking type Kind"
    $ do
      it "passes for variable types set in context"
        $ do
          cKind 0 [(Global "A", HasKind Star)] (TFree (Global "A")) Star
            `shouldBe` Right ()
      it "fails for variable types not set in context"
        $ do
          evaluate (cKind 0 [] (TFree (Global "A")) Star)
            `shouldThrow` anyException
      it "passes for function types with valid input and output variable types"
        $ do
          cKind
            0
        ...
        $ do
          iType
            ...
            (Ann
               ...
               (Fun (TFree (Global "A")) (TFree (Global "A"))))
            `shouldBe` Right (Fun (TFree (Global "A")) (TFree (Global "A")))
      it
