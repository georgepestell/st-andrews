...
helpTxt cs = "List of commands:  Any command may be abbreviated to :c where\n"
...
dummy = makeTokenParser
  (haskellStyle { identStart = letter <|> P.char '_', reservedNames = [] })

parseIO :: String -> CharParser () a -> String -> IO (Maybe a)
parseIO f p x =
  case P.parse (whiteSpace dummy >> p >>= \x -> eof >> return x) f x of
...
handleCommand int state@(out, ve, te) cmd = case cmd of
...
compileFile int state@(out, ve, te) f = do
  x <- readFile f
...
compilePhrase
  :: Interpreter i c v t tinf inf -> State v inf -> String -> IO (State v inf)
compilePhrase int state@(out, ve, te) x = do
...
handleStmt :: Interpreter i c v t tinf inf
           ...
           -> Stmt i tinf
           -> IO (State v inf)
handleStmt int state@(out, ve, te) stmt = do
  case stmt of
...
check :: Interpreter i c v t tinf inf
      ...
      -> ((t, v) -> State v inf)
      -> IO (State v inf)
check int state@(out, ve, te) i t kp k = do
  -- i: String, t: Type
